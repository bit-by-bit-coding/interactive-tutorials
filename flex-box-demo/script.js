...
buttonNumItems.onclick = function(){
    ...
    for (let i=1; i<=x; i++) {
        // I am so sorry
        sLeft += "<button onclick='showDiv(" + i.toString() + ")' id='button-item-" + i.toString() + "'>Item " + i.toString() + "</button> <br /><div id='left-item-" + i.toString() + "' class='hidden'><p><span class='leftCode'>order:</span> <input type='number' id='order-item-" + i.toString() + "'></p><p><span class='leftCode'>flex-grow:</span> <input type='number' id='flex-grow-item-" + i.toString() + "'></p><p><span class='leftCode'>flex-shrink:</span> <input type='number' id='flex-shrink-item-" + i.toString() + "'></p><p><span class='leftCode'>flex-basis:</span> <input type='number' id='flex-basis-item-" + i.toString() + "'> px</p><p><span class='leftCode'>align-self:</span> <select id='align-self-" + i.toString() + "'><option></option><option value='center'>center</option><option value='flex-start'>flex-start</option><option value='flex-end'>flex-end</option><option value='stretch'>stretch</option><option value='baseline'>baseline</option></select></p></div> <br />";
    }
